...
@section('title', 'Langkah Perhitungan SMART')
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb breadcrumb-style1 mb-0">
                ...
                <li class="breadcrumb-item">
                    <a href="{{ route('admin.perhitungan.smart.index') }}">Perhitungan SMART</a>
                </li>
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <h5 class="mb-0">
                    Langkah Perhitungan Metode SMART (Per Kelas)
                    <small class="text-muted">- {{ $tahunAjaran->tahun_ajaran }}</small>
                </h5>
                <a href="{{ route('admin.perhitungan.smart.index', ['tahun_ajaran' => $tahunAjaran->id_ta, 'semester' => $id_semester, 'kelas' => $selectedKelasIds ?? []]) }}"
            ...
            <div class="card-body">
                <div class="alert alert-info mb-3">
                    ...
                    <ol class="mb-0 mt-2">
                        <li>Normalisasi nilai berdasarkan jenis kriteria (Benefit/Cost) pada kelompok kelas terpilih</li>
                        <li>Kalikan nilai ternormalisasi dengan bobot kriteria</li>
                        <li>Jumlahkan semua nilai terbobot (Utility) sebagai skor akhir kelas</li>
                        <li>Ranking berdasarkan skor tertinggi dalam kelas</li>
                    </ol>
                </div>

                {{-- Class Selector Form --}}
                <form action="{{ route('admin.perhitungan.smart.steps', $tahunAjaran->id_ta) }}" method="GET" class="row g-3 align-items-center">
        ...
        @if ($step1Steps->total() > 0)
            ...
            <div class="card mb-4">
                <h5 class="card-header bg-label-info">
                    ...
                    <small class="d-block mt-1">(Benefit: (x - min) / (max - min) | Cost: (max - x) / (max - min))</small>
                </h5>
                <div class="table-responsive">
                    <table class="table table-bordered table-sm">
                        <thead class="table-light">
                        ...
                        <tbody>
                            @foreach ($step3Steps as $index => $step)
                                <tr>
                                    ...
                                    <td><strong>{{ $step['steps']['nama'] }}</strong></td>
                                    @foreach ($step['steps']['kriteria_details'] as $detail)
                                        <td class="text-center">{{ $detail['weighted_utility'] }}</td>
                                    @endforeach
                                </tr>
                            @endforeach
                        </tbody>
            ...
            <div class="card mb-4">
                <h5 class="card-header bg-label-success">Langkah 4: Perhitungan Skor Akhir &amp; Ranking Kelas {{ $activeKelas->nama_kelas ?? '' }}</h5>
                <div class="table-responsive">
                    <table class="table table-bordered table-sm">
                        <thead class="table-light">
                            <tr>
                                <th class="text-center" style="width: 50px;">No</th>
                                <th>Alternatif (Siswa)</th>
                                <th class="text-center">Skor SMART (Utility)</th>
        ...
        <div class="card">
            <div class="card-body text-center d-flex justify-content-center gap-2">
                ...
                <a href="{{ route('admin.perhitungan.finalis.smart.index', ['tahun_ajaran' => $tahunAjaran->id_ta, 'semester' => $id_semester]) }}" class="btn btn-primary">
