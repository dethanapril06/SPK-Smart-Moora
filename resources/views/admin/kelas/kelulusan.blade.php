    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="alert alert-warning mb-4" role="alert">
            <div class="d-flex align-items-start">
                ...
                <div>
                    <strong>Petunjuk Kelulusan Siswa Kelas XII:</strong>
                    <ul class="mb-0 ps-3 mt-1 text-muted">
                        <li>Pilih <strong>Tahun Ajaran</strong> dan <strong>Kelas XII</strong> yang siswanya akan diluluskan.</li>
                        <li>Siswa yang diluluskan akan ditandai sebagai <span class="badge bg-success">Lulus</span>, tidak lagi terdaftar di kelas manapun, dan data nilai tetap tersimpan sebagai arsip.</li>
                        <li>Proses ini <strong>tidak dapat dibatalkan</strong>. Pastikan data siswa sudah benar sebelum menyimpan.</li>
                        <li>Disarankan menyelesaikan proses kelulusan kelas XII terlebih dahulu <strong>sebelum</strong> menaikkan siswa dari kelas XI ke kelas XII.</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form action="{{ route('admin.kelas.kelulusan.index') }}" method="GET">
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-md-4">
                            <label class="form-label" for="id_kelas">Kelas XII</label>
                            <select class="form-select" id="id_kelas" name="id_kelas" required>
                                <option value="">-- Pilih Kelas --</option>
                                @foreach ($kelasList as $kelas)
                                    <option value="{{ $kelas->id_kelas }}" {{ $sourceKelasId === $kelas->id_kelas ? 'selected' : '' }}>
                                        {{ $kelas->nama_kelas }}
                                    </option>
                                @endforeach
                            </select>
        ...
        {{-- Student Table --}}
        @if ($sourceTahunAjaran && $sourceKelasId)
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div>
                        <h5 class="mb-0">
                            Daftar Siswa &mdash;
                            <span class="text-primary">{{ $selectedKelas?->nama_kelas ?? $sourceKelasId }}</span>
                ...
                @else
                    <form action="{{ route('admin.kelas.kelulusan.store') }}" method="POST" id="form-kelulusan">
                        @csrf
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p class="mb-2">
                                        ...
                                        <strong>{{ $selectedKelas?->nama_kelas ?? $sourceKelasId }}</strong>
                                        Tahun Ajaran <strong>{{ $sourceTahunAjaran }}</strong>.
                                    </p>
                                    <div class="alert alert-danger py-2 mb-0">
                                        <i class="bx bx-info-circle me-1"></i>
                                        <strong>Proses ini tidak dapat dibatalkan.</strong>
                                        Status siswa akan berubah menjadi <strong>Lulus</strong> dan tidak dapat diaktifkan kembali melalui halaman ini.
                                    </div>
                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateSelectedCount() {
            ...
            selectedCountLabel.textContent = selected + ' siswa dipilih';
            btnLulus.disabled = selected === 0;
