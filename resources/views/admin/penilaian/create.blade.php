                    ...
                    @foreach ($kriteriaList as $kriteria)
                        <div class="card mb-3 kriteria-section">
                            <div class="card-body">
                                        ...
                                        <h6 class="mb-0">
                                            ...
                                            @if ($kriteria->kode_kriteria == 'C5')
                                                <span class="badge bg-label-warning ms-2">
                                                    <i class="bx bx-bolt"></i> Otomatis
                                                </span>
                                            @endif
                                        </h6>
                                        <small class="text-muted">
                                            Jenis: <strong>{{ $kriteria->jenis_kriteria }}</strong> |
                                ...
                                @if ($kriteria->kode_kriteria == 'C5')
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <small class="text-muted">Dihitung otomatis dari risayat pelanggaran</small>
                                        </div>
                                        <div class="col-md-6">
                                ...
                                @else
                                    <!-- Other Kriteria: Manual Input -->
                                    <div class="row">
                                        <div class="col-md-12">
                                            <label class="form-label"
                                                for="nilai_asli_{{ strtolower($kriteria->kode_kriteria) }}">
                                                Nilai Asli (0-100) <span class="text-danger">*</span>
                                            </label>
                                            <input type="number" step="0.01" min="0" max="100"
                                                ...
                                                required>
                                            <small class="text-muted">
                                                Akan dikonversi ke sub kriteria berdasarkan rentang nilai
                                            </small>
                                        </div>
                                    </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function updateC5() {
                    ...
                    fetch(`/admin/penilaian/get-c5?id_siswa=${siswaId}&id_ta=${taId}`)
                ...
                // Auto-convert nilai when input changes
                document.querySelectorAll('.nilai-input').forEach(input => {
                    input.addEventListener('input', function() {
                        ...
                        // Find matching sub kriteria
                        const subKriteria = subKriteriaData[kriteriaId];
                        if (!subKriteria) return;

                        const matched = subKriteria.find(sk =>
                            parseFloat(sk.nilai_awal) <= nilai && parseFloat(sk.nilai_akhir) >=
                            nilai
                        );
                        ...
                        if (matched) {
                            ...
                            konversiText.textContent = 'Tidak ada sub kriteria yang sesuai';
                            konversiDiv.style.display = 'block';
                        }
                    });
                });

                // Initial C5 update if values are pre-selected
                if (siswaSelect.value && taSelect.value) {
                    updateC5();
                }
            });
        </script>
