...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            <h5 class="card-header">Form Tambah Riwayat Pelanggaran</h5>
            <div class="card-body">
                <form action="{{ route('walikelas.riwayatpelanggaran.store') }}" method="POST">
                    @csrf
                                ...
                                <select id="id_ta" name="id_ta"
                                    ...
                                    @foreach ($tahunAjaranList as $ta)
                                        <option value="{{ $ta->id_ta }}"
                                            {{ (old('id_ta') ?? ($tahunAjaranAktif ? $tahunAjaranAktif->id_ta : '')) == $ta->id_ta ? 'selected' : '' }}>
                                ...
                                <select id="id_semester" name="id_semester"
                                    ...
                                    @foreach ($semesterList as $s)
                                        <option value="{{ $s->id_semester }}" data-id-ta="{{ $s->id_ta }}"
                                            {{ (old('id_semester') ?? ($tahunAjaranAktif ? $tahunAjaranAktif->id_semester : '')) == $s->id_semester ? 'selected' : '' }}>
                                            {{ $s->nama_semester }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select id="kategori_filter" class="form-select">
                                    ...
                                    @foreach ($kategoriList as $kat)
                                        <option value="{{ $kat }}">{{ str_replace('_', ' ', $kat) }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                @error('id_jenis_pelanggaran')
                                    <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                                <div id="bobot_display" class="mt-2" style="display: none;">
                                    <small class="text-muted">Bobot Poin: <span id="bobot_value"
                                            class="badge"></span></small>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="keterangan_tambahan" name="keterangan_tambahan"
                            ...
                            placeholder="Tambahan informasi atau catatan...">{{ old('keterangan_tambahan') }}</textarea>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                kategoriFilter.addEventListener('change', function() {
                    ...
                    fetch(
                            `{{ route('walikelas.riwayatpelanggaran.getJenisPelanggaran') }}?kategori=${kategori}`)
