    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb breadcrumb-style1 mb-0">
                ...
                <li class="breadcrumb-item">
                    <a href="{{ route('walikelas.nilaiekstrakurikuler.index') }}">Nilai Ekstrakurikuler</a>
                </li>
        ...
        <div class="card">
            <h5 class="card-header">Form Tambah Nilai Ekstrakurikuler - {{ $kelas->nama_kelas }}</h5>
            <div class="card-body">
                <form action="{{ route('walikelas.nilaiekstrakurikuler.store') }}" method="POST" id="formEkskul">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        @error('id_siswa')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                        @if ($selectedTA && $siswaList->count() == 0)
                            <div class="form-text text-warning">Tidak ada siswa ditemukan untuk tahun ajaran ini.</div>
                        @endif
                    </div>
                    ...
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('walikelas.nilaiekstrakurikuler.index', ['tahun_ajaran' => $selectedTA]) }}"
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                // Reload page when TA changes to load siswa list
                $('#id_ta').on('change', function() {
                    var ta = $(this).val();
                    if (ta) {
                        window.location.href = "{{ route('walikelas.nilaiekstrakurikuler.create') }}" +
                            "?tahun_ajaran=" + ta;
                    }
                });
                ...
                $('#id_siswa').on('change', function() {
                    ...
                    $.ajax({
                        url: "{{ route('walikelas.nilaiekstrakurikuler.getEkskul') }}",
                        type: 'GET',
                        data: {
                            id_siswa: idSiswa,
                            id_ta: idTa
                ...
                // Trigger load jika sudah ada siswa yang dipilih
                if ($('#id_siswa').val()) {
