    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="breadcrumb breadcrumb-style1 mb-0">
                ...
                <li class="breadcrumb-item"><a href="{{ route('walikelas.perhitungan.smart.index') }}">Perhitungan SMART</a></li>
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    Langkah Perhitungan Metode SMART
                    <small class="text-muted">- {{ $tahunAjaran->tahun_ajaran }} {{ $tahunAjaran->semester }}</small>
                </h5>
                <a href="{{ route('walikelas.perhitungan.smart.index', ['tahun_ajaran' => $tahunAjaran->id_ta, 'semester' => $id_semester]) }}"
        ...
        <div class="card mb-4">
            <h5 class="card-header bg-label-info">
                Langkah 2: Matrix Normalisasi
                <small>(ni = (xi - xmin) / (xmax - xmin) untuk Benefit, ni = (xmax - xi) / (xmax - xmin) untuk Cost)</small>
            </h5>
            <div class="table-responsive">
                <table class="table table-bordered table-sm">
                    <thead class="table-light">
        ...
        <div class="card mb-4">
            <h5 class="card-header bg-label-warning">
                Langkah 3: Matrix Pembobotan <small>(Normalized × Weight)</small>
            </h5>
            <div class="card-body pb-2">
                <p class="mb-2"><strong>Bobot Kriteria:</strong></p>
                <div class="d-flex gap-2 flex-wrap mb-3">
                    @foreach ($kriteriaList as $kriteria)
                        <span class="badge bg-label-secondary">{{ $kriteria->kode_kriteria }}: {{ $kriteria->bobot }}</span>
                    @endforeach
                </div>
            </div>
            <div class="table-responsive">
                    ...
                    <tbody>
                        @foreach ($step4Steps as $index => $step)
                            <tr>
                                ...
                                <td class="text-center">
                                    <span class="badge bg-primary">{{ $step['rank_smart'] }}</span>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
